  ...
  return (
    <section className="bg-black text-white py-16 px-4">
      <div className="text-center mb-12">
        <h2 className="text-[60px] mb-2">Customer Reviews</h2>
        <p className="text-[28px] text-gray-300">
          Real stories from real fashion lovers.
        </p>
      </div>
      <div className="grid md:grid-cols-3 gap-8 w-full px-4 md:px-8 divide-x divide-white">
        {reviews.map((item, idx) => (
          <div key={idx} className="px-6 py-8 text-center space-y-4">
            <img
              src={item.image}
              alt={item.name}
              className="w-[84px] h-[84px] rounded-full mx-auto object-cover"
            />
            <p className="font-medium text-[28px]">
              {item.name} , {item.age}
            </p>
            <p className="text-white text-[28px] leading-relaxed">
              “{item.review}”
            </p>
          </div>
        ))}
